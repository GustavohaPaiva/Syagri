...
export function FornecedorFiltersPanel({
  searchQuery,
  onSearchChange,
  onClear,
  hasFilters,
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm sm:rounded-3xl">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Encontre fornecedores pelo nome.
            </p>
          </div>
          {hasFilters ? (
            <Button
      ...
      <div className="p-4 sm:p-6">
        <label
          htmlFor="fornecedor-filter-nome"
        ...
        <SearchInput
          id="fornecedor-filter-nome"
          ariaLabel="Buscar fornecedor por nome"
          placeholder="Ex.: Yara, Mosaic…"
          value={searchQuery}
...
export function FornecedorStatsBar({ total, filtered, ativos, loading }) {
  const items = [
    {
      label: 'Fornecedores',
      value: loading ? '—' : String(total),
      hint: 'Cadastrados na operação',
      icon: IconUsers,
    ...
    },
    {
      ...
      hint: 'Disponíveis para importação',
      icon: IconUsers,
...
function fornecedorInitial(nome) {
  const trimmed = (nome ?? '').trim()
  return trimmed ? trimmed.charAt(0).toUpperCase() : '?'
}

export function FornecedorNameBadge({ nome }) {
  return (
    <span className="inline-flex max-w-full items-center gap-2">
      <span
        className="flex size-7 shrink-0 items-center justify-center rounded-full bg-sky-50 text-xs font-semibold text-sky-700 ring-1 ring-sky-100"
        aria-hidden
      >
        {fornecedorInitial(nome)}
      </span>
